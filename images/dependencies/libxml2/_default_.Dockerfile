...
################################################################################
FROM $BASE_OS_IMAGE AS libxml2-builder
ARG LIBXML2_VERSION
...
ENV LDFLAGS="-Wl,-rpath,${LIBXML2_INSTALL_PREFIX}/lib,-rpath,${ZLIB_INSTALL_PREFIX}/lib" \
    CC=clang \
    CXX=clang++ \
    CFLAGS=-O2
RUN ./autogen.sh \
...
cp ./Copyright "${LICENSES_DIR}/libxml2.txt"
cat <<_LICENSE_METADATA_ >"${LICENSES_DIR}/metadata/libxml2.json"
{
    "name": "libxml2",
    "url": "https://gitlab.gnome.org/GNOME/libxml2",
    "version": "${LIBXML2_VERSION}",
    "filenames": {
        "Copyright": "libxml2.txt"
    }
}
_LICENSE_METADATA_
